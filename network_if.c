...
// keep state of network related things here, to be accessed by simplelink functions
static struct NetworkState net_state;
...
void SimpleLinkWlanEventHandler(SlWlanEvent_t *pSlWlanEvent)
{
    ...
    case SL_WLAN_EVENT_CONNECT:
        SET_STATUS_BIT(net_state.g_ulStatus, STATUS_BIT_CONNECTION);

        memcpy(net_state.g_ucConnectionSSID, pSlWlanEvent->Data.Connect.SsidName,
               pSlWlanEvent->Data.Connect.SsidLen);
        memcpy(net_state.g_ucConnectionSSID, pSlWlanEvent->Data.Connect.Bssid,
               SL_WLAN_BSSID_LENGTH);

        UART_PRINT(
            "[WLAN EVENT] STA Connected to the AP: %s , BSSID: "
            "%x:%x:%x:%x:%x:%x\n\r", net_state.g_ucConnectionSSID,
            net_state.g_ucConnectionSSID[0], net_state.g_ucConnectionSSID[1],
            net_state.g_ucConnectionSSID[2],
            net_state.g_ucConnectionSSID[3], net_state.g_ucConnectionSSID[4],
            net_state.g_ucConnectionSSID[5]);
        break;

    case SL_WLAN_EVENT_DISCONNECT:
        CLR_STATUS_BIT(net_state.g_ulStatus, STATUS_BIT_CONNECTION);
        CLR_STATUS_BIT(net_state.g_ulStatus, STATUS_BIT_IP_ACQUIRED);
    ...
    case SL_WLAN_EVENT_STA_REMOVED:
        ...
        CLR_STATUS_BIT(net_state.g_ulStatus, STATUS_BIT_CONNECTION);
        CLR_STATUS_BIT(net_state.g_ulStatus, STATUS_BIT_IP_LEASED);
        break;
...
void SimpleLinkNetAppEventHandler(SlNetAppEvent_t *pNetAppEvent)
{
    ...
    case SL_NETAPP_EVENT_IPV6_ACQUIRED:
        SET_STATUS_BIT(net_state.g_ulStatus, STATUS_BIT_IP_ACQUIRED);
        UART_PRINT("[NETAPP EVENT] IP acquired by the device\n\r");
        break;

    case SL_NETAPP_EVENT_DHCPV4_LEASED:
        SET_STATUS_BIT(net_state.g_ulStatus, STATUS_BIT_IP_LEASED);
        net_state.g_ulStaIp = (pNetAppEvent)->Data.IpLeased.IpAddress;

        UART_PRINT("[NETAPP EVENT] IP Leased to Client: IP=%d.%d.%d.%d , ",
                   SL_IPV4_BYTE(net_state.g_ulStaIp,
                                3),
                   SL_IPV4_BYTE(net_state.g_ulStaIp,
                                2),
                   SL_IPV4_BYTE(net_state.g_ulStaIp, 1), SL_IPV4_BYTE(net_state.g_ulStaIp, 0));
        break;
    ...
    case SL_NETAPP_EVENT_DHCPV4_RELEASED:
        ...
        UART_PRINT("[NETAPP EVENT] IP Released for Client: "
                   "IP=%d.%d.%d.%d , ", SL_IPV4_BYTE(net_state.g_ulStaIp,
                                                     3),
                   SL_IPV4_BYTE(net_state.g_ulStaIp,
                                2),
                   SL_IPV4_BYTE(net_state.g_ulStaIp, 1), SL_IPV4_BYTE(net_state.g_ulStaIp, 0));
...
{
    net_state.g_ulStatus = 0;
    net_state.g_ulStaIp = 0;

    memset(net_state.g_ucConnectionSSID, 0, sizeof(net_state.g_ucConnectionSSID));
    memset(net_state.g_ucConnectionSSID, 0, sizeof(net_state.g_ucConnectionSSID));
}
    ...
    if(uiMode != lRetVal)
    {
        ...
        if(lRetVal == uiMode)
        {
            ...
            case ROLE_AP:
                ...
                while(!IS_IP_ACQUIRED(net_state.g_ulStatus))
                {
                    usleep(1000);
                }
                break;
                          ...
                          SlWlanSecParams_t SecurityParams)
{
    ...
    // initialize network state struct
    net_state.g_ulStaIp = 0;
    net_state.g_ulStatus = 0;

    net_state.g_usConnectIndex = 0;
    ...
    if(pcSsid != NULL)
    {
        ...
        while(net_state.g_usConnectIndex < 10)
        {
            sleep(1);

            if(IS_CONNECTED(net_state.g_ulStatus) && IS_IP_ACQUIRED(net_state.g_ulStatus))
            {
                break;
            }
            net_state.g_usConnectIndex++;
        }
    }
    else
    {
    ...
    while(!(IS_CONNECTED(net_state.g_ulStatus)) || !(IS_IP_ACQUIRED(net_state.g_ulStatus)))
    {
        ...
        usConnTimeout = net_state.g_usConnectIndex + 15;
        ...
        while(!(IS_CONNECTED(net_state.g_ulStatus)) || !(IS_IP_ACQUIRED(net_state.g_ulStatus)))
        {
            ...
            if(net_state.g_usConnectIndex >= usConnTimeout)
            {
                break;
            }
            net_state.g_usConnectIndex++;
...
long Network_IF_DisconnectFromAP(void)
{
    ...
    if(IS_CONNECTED(net_state.g_ulStatus))
    {
        lRetVal = sl_WlanDisconnect();
        if(0 == lRetVal)
        {
            while(IS_CONNECTED(net_state.g_ulStatus))
            {
                usleep(1000);
            }
            return(lRetVal);
...
{
    SET_STATUS_BIT(net_state.g_ulStatus, cStat);
}
